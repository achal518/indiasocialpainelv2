...
function openGenericUPIPayment(amount) {
    try {
        ...
        // Is baar, hum transaction note ko hata rahe hain taaki koi galti na ho
        const upiURL = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(UPI_CONFIG.businessName)}&am=${amount}&cu=INR`;
        ...
        setTimeout(() => {
            if (new Date().getTime() - startTime < 1000) {
                showNotification('achal to open UPI app. Please try again.', 'error');
            } else {
                showNotification('achal app opened successfully! Complete payment in your UPI app.', 'success');
            }
...
window.openUPIAppGeneral = function() {
  ...
  if (amount >= 100 && amount <= 100000) {
    ...
    const upiID = 'kavita.5049-4@waicici';
...
function showQRCodeModal(order) {
  ...
  window.generateActualQRCode = function() {
    ...
    // Amount ko input field se liya ja raha hai, taaki Add Fund page par bhi kaam kare
    ...
    if (qrContainer && generateBtn) {
        const upiID = UPI_CONFIG.upiID; // UPI ID ko UPI_CONFIG se liya gaya hai
        
        // Yah hai woh sahi UPI string
...
function showUPIIDModal(order) {
  ...
  const actualUPIID = 'kavita.5049-4@waicici'; // Real UPI ID for copying - consistent with app opening
